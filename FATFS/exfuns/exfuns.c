...
//返回值:0,正常
//    其他,错误,0XFF,强制退出
u8 exf_copy(u8 *psrc,u8 *pdst,u32 totsize,u32 cpdsize,u8 fwmode)
{
	u8 res;
    u16 br=0;
	u16 bw=0;
...
//返回值:0,成功
//    其他,错误代码;0XFF,强制退出
u8 exf_fdcopy(u8 *psrc,u8 *pdst,u32 *totsize,u32 *cpdsize,u8 fwmode)
{
#define MAX_PATHNAME_DEPTH	512+1	//最大目标文件路径+文件名深度
	u8 res=0;	  
    DIR *srcdir=0;		//源目录
...
// 				fcpymsg(fn,0,0X04);//更新文件夹名
				...
				while(res==0)//开始复制文件夹里面的东东
				{
 					...
 					if(finfo->fattrib&0X10)//是子目录(文件属性,0X20,归档文件;0X10,子目录;)
					{
						...
						res=exf_fdcopy(srcpathname,dstpathname,totsize,cpdsize,fwmode);	//拷贝文件夹
					}else //非目录
					{
...
// 						fcpymsg(fn,0,0X01);//更新文件名
						res=exf_copy(srcpathname,dstpathname,*totsize,*cpdsize,fwmode);//复制文件
...
const u8* SYSTEM_APPPATH="0:/WZTSYSTEM/APP";	//APP文件夹路径
const u8* SYSTEM_FONTPATH="0:/WZTSYSTEM/FONT";	//FONT文件夹路径
const u8* SYSTEM_SPBPATH="0:/WZTSYSTEM/SPB";	//SPB文件夹路径
const u8* SYSTEM_DSTPATH="1:/WZTSYSTEM";		//系统文件目标路径
//系统更新
//返回值:0,正常
//    其他,错误,0XFF强制退出了
u8 asystem_update(void)
{
	u32 totsize=0;
	u32 cpdsize=0;
	u8 res=0;
	totsize+=exf_fdsize((u8*)SYSTEM_APPPATH);//得到三个文件夹的总大小
	totsize+=exf_fdsize((u8*)SYSTEM_FONTPATH);
	totsize+=exf_fdsize((u8*)SYSTEM_SPBPATH);
	f_mkdir((const TCHAR *)SYSTEM_DSTPATH);//强制创建目标文件夹
 	res=exf_fdcopy((u8*)SYSTEM_APPPATH,(u8*)SYSTEM_DSTPATH,&totsize,&cpdsize,1);
	if(res)
	{
		if(res!=0xff)return 1;	//第一阶段复制出错
		else return 0XFF;		//强制退出
	}
	res=exf_fdcopy((u8*)SYSTEM_FONTPATH,(u8*)SYSTEM_DSTPATH,&totsize,&cpdsize,1);
	if(res)
	{
		if(res!=0xff)return 1;	//第二阶段复制出错
		else return 0XFF;		//强制退出
	}
 	res=exf_fdcopy((u8*)SYSTEM_SPBPATH,(u8*)SYSTEM_DSTPATH,&totsize,&cpdsize,1);
	if(res)
	{
