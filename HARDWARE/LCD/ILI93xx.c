...
//teng
//亮转暗
u16 Color_Day_To_Night(u16 color) {
    u16 result;
    u8 R, G, B;
    //获取高字节的5个bit
    R = color & 0xF800;//32
    ...
    //获取低字节5个bit
    B = color & 0x001F;//32
    result = (((32-R)<<11) & ((64-G)<<5) & ((32-B)<<0));
    return result;
}

//teng
//暗转亮
u16 Color_Night_To_Day(u16 color) {
    return Color_Day_To_Night(color);
}
...
//LCD写GRAM
//RGB_Code:颜色值
void LCD_WriteRAM_Color(u16 RGB_Code)
{						
#ifdef TURN_COLOR
    RGB_Code = Color_Day_To_Night(RGB_Code);
#endif    
...
#ifdef TURN_COLOR
        return Color_Night_To_Day(((r>>11)<<11)|((g>>10)<<5)|(b>>11));
#else 
        return (((r>>11)<<11)|((g>>10)<<5)|(b>>11));//ILI9341/NT35310/NT35510需要公式转换一下
#endif
    } else {
#ifdef TURN_COLOR
        return Color_Night_To_Day(LCD_BGR2RGB(r));
#else 
...
//	TFTLCD->LCD_RAM=POINT_COLOR;//teng
    LCD_WriteRAM_Color(POINT_COLOR);//teng
}
//快速画点
//x,y:坐标
//color:颜色
...
#endif    
	TFTLCD->LCD_REG=lcddev.wramcmd; 
//	TFTLCD->LCD_RAM=color;//teng
    LCD_WriteRAM_Color(color);//teng
}	 
//SSD1963 背光设置
//pwm:背光等级,0~100.越大越亮.
...
#endif
	LCD_Display_Dir(TFTLCD_DIR_MODE);		//横屏    0,竖屏；1,横屏
	LCD_LED=1;				//点亮背光
	LCD_Clear(BLACK);
//    LCD_ShowString(92, 152, 56, 16, 16, "Welcome", 1, BLACK);
//    delay_ms(1000);
}  
//清屏函数
//color:要清屏的填充色
...
{          
 	...
 	}else
	{
		...
		for(i=sy;i<=ey;i++)
		{
			...
			LCD_WriteRAM_Prepare();     			//开始写入GRAM	  
			for(j=0;j<xlen;j++) {
//                TFTLCD->LCD_RAM=color;	//显示颜色  teng
                LCD_WriteRAM_Color(color);//teng
            }                
		}
	}	 
...
{  
 	...
 	for(i=0;i<height;i++)
	{
		...
		LCD_WriteRAM_Prepare();     //开始写入GRAM
		for(j=0;j<width;j++) {
//            TFTLCD->LCD_RAM=color[i*width+j];//写入数据  teng
            LCD_WriteRAM_Color(color[i*width+j]);//teng
        }
	}		  
}  
//画线
...
//color:颜色 
//num:数值(0~4294967295);	 
void LCD_ShowNum(u16 x,u16 y,u32 num,u8 len,u8 size, u8 mode, u16 color)
{         	
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
...
//size:字体大小
//*p:字符串起始地址		  
void LCD_ShowString(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p, u8 mode, u16 color)
{         
	u8 x0=x;
	width+=x;
	height+=y;
