...
TREEVIEW_ITEM_Handle   vhTreeItemSel;
TREEVIEW_ITEM_Handle   vhTreeItemInc;
TREEVIEW_ITEM_Handle   vhTreeItemDec;
WM_HWIN vhWinx;
WM_HWIN vhItemTree;
u8 vpathbuffx[150]={0};
TREEVIEW_Handle vtreeitem;
...
static const GUI_WIDGET_CREATE_INFO _aDialogCreatex[] = {
	{ FRAMEWIN_CreateIndirect, "视频浏览器", ID_FRAMEWIN_0, 0, 0, 240, 320, 0, 0x00, 0 },
...
//树形文件初始化
WM_HWIN vTREEViewInitx(WM_MESSAGE * pMsg,U16 ID)
{
	WM_HWIN hItem;
...
//	Fill_FileList("1:",hItem,hTreeItemNew,NULL);
	...
	Fill_FileList("0:",hItem,hTreeItemNew,NULL,T_AVI);                                    //扫描SDCard文件
	TREEVIEW_AttachItem(hItem, hTreeItemNew, hTreeItemCur, TREEVIEW_INSERT_BELOW);   //增加到树形图

	hTreeItemCur = TREEVIEW_GetItem(hItem, 0, TREEVIEW_GET_FIRST);
	Fill_FileList("1:",hItem,hTreeItemCur,NULL,T_AVI);                                    //扫描ExFlash文件
	TREEVIEW_AttachItem(hItem, hTreeItemNew, hTreeItemCur, TREEVIEW_INSERT_BELOW);   //增加到树形图
	return hItem;
}
...
//u8 pathbuffx[150]={0};
//树形文件路径显示，并返回盘符名称句柄
TREEVIEW_ITEM_Handle vTREEVIEWShowPathx(WM_MESSAGE *pMsg, U16 TREEVIEW_IDx, U16 EDIT_IDx)
{
	u8 pbuff[150]={0};
	u8 pbufft[150]={0};
	u8 pathbuff[150]={0};
...
//	TREEVIEW_ITEM_Handle   hTreeItemSel;
	...
	strcpy(vpathbuffx,pathbuff);
TREEVIEW_SetSel(vtreeitem, vhTreeItemSel);    //设置回点击item
//TREEVIEW_IncSel(treeitem);                //测试用
//TREEVIEWShowPathx(pMsg,ID_TREEVIEW_0,ID_EDIT_0);   //测试用
	return vhTreeItemSel;
}
...
//	MENU_ITEM_DATA         ItemData;
	...
	switch (pMsg->MsgId) {
		...
		//	WM_DisableWindow(hItem);           //禁用状态
			...
			//
			hItem = vTREEViewInitx(pMsg,ID_TREEVIEW_0); //树形文件初始化
			// USER END
		break;
				...
				case ID_TREEVIEW_0: // Notifications sent by 'Treeview'
					switch(NCode) {
						...
						// USER START (Optionally insert code for reacting on notification message)
							...
							vhTreeItemSel = vTREEVIEWShowPathx(pMsg,ID_TREEVIEW_0,ID_EDIT_0);					//根据选中条目显示路径

							hItem = WM_GetDialogItem(pMsg->hWin, ID_TREEVIEW_0);                            //获取树形句柄
							TREEVIEW_ITEM_GetText(vhTreeItemSel,pbufft,50);                                 //获取选中文本
							if( (f_typetell(pbufft)>>4) == (T_AVI>>4) ) {
								if(strcmp(vpbuffx,pbufft)==0) {
									vfilesameflag = 1;
								} else {
									strcpy(vpbuffx,pbufft);
									vfilesameflag = 0;
								}
								if (vfilesameflag==1) {  //第二次单击则打开文件
                                    runVideo(vpathbuffx,pbufft);// 运行
								}
							}
